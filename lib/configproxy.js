...
class ConfigurableProxy extends EventEmitter {
  ...
  addRoute(path, data) {
    ...
    log.debug("addRoute: %s", JSON.stringify({ path: path, data: data}));
  ...
  postRoutes(req, res, path, data) {
    ...
    // TODO: allow null to mean no auth required?
    if (this.options.requireAuthToken && typeof data.authToken !== "string") {
      log.warn("Bad POST data: %s", JSON.stringify(data));
      fail(req, res, 400, "Must specify 'authToken' as string");
      return;
    }
    ...
    return this._routes.getTarget(path).then(function(route) {
      ...
      if (route) {
        return {
          ...
          authToken: route.data.authToken,
        };
      }
    });
  }
  ...
  handleProxy(kind, req, res) {
    ...
    return this.targetForReq(req).then(function(match) {
      ...
      if (that.options.requireAuthToken) {
        var tokre = (req.headers.authorization || "").match(/token\s+(\S+)/);
        var token = (tokre !== null) ? tokre[1] : null;
        if (!match.authToken || token !== match.authToken) {
          log.info("Rejecting web request from: %s", req.headers.authorization || "");
          res.writeHead(403);
          res.end();
          return;
        }
